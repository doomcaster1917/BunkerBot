    ...
    while True:
        ...
        if decision.lower() == "Да".lower():
            ...
            if user_login.lower() == 'Отказаться'.lower():
                break
            user_password = input("Введите пароль. Чтобы отказаться, введите: Отказаться.")

            if user_password.lower() == 'Отказаться'.lower():
                break
            admin_id = input("Введите id вашей страницы, на которую дополнительная страница будет слать уведомления."
                             "На странице должна быть отключена двухфакторная авторизация."
